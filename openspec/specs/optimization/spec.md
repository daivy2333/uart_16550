...
### Requirement: 已识别优化点（O1-O8，O6/O9 已归档）

优化记录 MUST 包含 O1 至 O8 八项已知优化点，每项含问题描述、当前影响、建议方案。
O6（批量读写 API）和 O9（异步串口集成）已归档至 ARC-202606241146，源文档不再保留。
...
- **THEN** 可以参考本 spec 的优先级标注：
  ...
  - **低优先级**: O7（FIFO 深度可配置化）
  - **已归档**: O6（批量读写）/ O9（异步串口集成）→ ARC-202606241146
- **AND** 应优先处理 StarryOS 集成相关的优化（O8）

---

<!-- arc: ARC-202606241146 --> 2 条已归档 (O6, O9) (2026-06-24) → ./changes/ARC-202606241146/proposal.md
